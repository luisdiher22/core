...
def _get_cover_entities(hass: HomeAssistant) -> dict[str, str]:
    ...
    Searches both the entity registry and state registry for cover entities
    that are enabled and have current states, which could potentially be garage doors.
    ...
    # First, check all current states for cover entities
    for state in hass.states.async_all():
        if state.entity_id.startswith("cover.") and state is not None:
            friendly_name = state.attributes.get("friendly_name", state.entity_id)
            covers[state.entity_id] = friendly_name

    # Also check entity registry for additional covers
    entity_registry = er.async_get(hass)
    for entity in entity_registry.entities.values():
        # Only include enabled cover entities that aren't already in our list
        if (
            entity.domain == "cover"
            and entity.disabled_by is None
            and entity.entity_id not in covers
        ):
...
def _get_barrier_entities(hass: HomeAssistant) -> dict[str, str]:
    ...
    # First, check all current states for barrier entities
    for state in hass.states.async_all():
        domain = state.entity_id.split(".")[0]
        if domain in barrier_domains and state is not None:
            friendly_name = state.attributes.get("friendly_name", state.entity_id)
            barriers[state.entity_id] = friendly_name

    # Also check entity registry for additional entities
    entity_registry = er.async_get(hass)
    for entity in entity_registry.entities.values():
        # Only include enabled entities from barrier domains that aren't already in our list
        if (
            entity.domain in barrier_domains
            and entity.disabled_by is None
            and entity.entity_id not in barriers
        ):
            state = hass.states.get(entity.entity_id)
            if state:
                friendly_name = state.attributes.get("friendly_name", entity.entity_id)
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            if not errors:
                ...
                garage_state = self.hass.states.get(garage_door)
                garage_name = garage_door
                if garage_state:
                    garage_name = garage_state.attributes.get(
